...
const cumulativeForksCount = computed<number>(() => {
  const cumulativeForks = (cumulativeData.value as ForksData)?.data;

  return cumulativeForks && cumulativeForks.length > 0
    ? cumulativeForks[cumulativeForks.length - 1]!.forks : 0;
});

...
import type {ActivityTypeItem} from "~~/types/development/responses.types";
...
const { data: activityTypesData } = await useFetch(() => {
  ...
  params.set('includeCollaborations', String(props.includeCollaborations ?? false));
  params.set('includeOtherContributions', String(props.includeOtherContributions ?? false));

  return `/api/project/${projectSlug.value}/activity-types?${params.toString()}`;
}, {
  watch: [() => props.includeCollaborations, () => props.includeOtherContributions],
  default: () => ({}),
});

...
const {
project, repository, startDate, endDate, selectedTimeRangeKey
...
const baseUrl = computed(() => {
  const url = `${domain.value}/embed/project/${project.value?.slug}`;
  if(repository.value?.slug) {
    return `${url}/repository/${repository.value?.slug}`;
  }
  return url
});

...
const {
  ...
  error,
  suspense
} = EXPLORE_API_SERVICE.fetchTopOrganizations();

const tableData = computed(() => {
  if (props.isFullList) {
    return data.value?.pages.flatMap((p) => (p as Pagination<ExploreOrganizations>).data);
  }
  return (data.value?.pages[0] as Pagination<ExploreOrganizations>).data;
});

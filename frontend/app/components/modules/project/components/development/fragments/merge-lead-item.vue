<template>
  <div class="flex flex-row gap-4 items-center justify-between">
    ...
    <div class="flex gap-2 items-center text-sm h-6 px-2.5 py-1 rounded-full" :class="valueClass">
      <lfx-icon :name="itemValue.changeType === 'positive' ? 'thumbs-up' : 'thumbs-down'" :size="14" />
      {{ itemValue.value }}
      {{ itemValue.unit }}
    </div>
  </div>

...
class BadgesApiService {
  ...
  private convertToBadges(leaderboards: Leaderboard[]): ProjectBadge[] {
    ...
    // Get leaderboard keys that have badge configs
    const validLeaderboardKeys = badgeConfigs.map((c) => c.leaderboardKey);

    // Filter to only leaderboards with badge configs
    const filteredLeaderboards = leaderboards.filter((l) =>
      validLeaderboardKeys.includes(l.leaderboardType),
    );

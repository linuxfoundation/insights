<template>
  <div>
    ...
    <div class="p-3 hidden sm:block">
      <lfx-tabs v-model="tab" :tabs="tabs" />
    </div>

    <div class="flex flex-col gap-1">
      <!-- Projects -->
      <div v-if="!tab && props.projects.length" class="pt-3 px-3 text-xs font-semibold leading-5 text-neutral-400">
        Projects
      </div>
      <section v-if="!tab || tab === 'projects' && props.projects.length > 0" class="flex flex-col gap-1">
        <nuxt-link
      ...
      <section v-if="tab === 'projects' && props.projects.length === 0" class="px-3 py-12 flex flex-col items-center">
        <lfx-icon name="laptop-code" :size="40" class="text-neutral-300" />
      ...
      <section v-if="!tab || tab === 'repositories' && props.repositories.length > 0" class="flex flex-col gap-1">
        <nuxt-link
          ...
          class="px-3 py-2 rounded-md transition-all
        hover:bg-neutral-50 flex items-center gap-2 cursor-pointer text-sm text-neutral-900">
          <lfx-icon name="book" :size="16" class="text-neutral-400" />
          {{repository.name}}
        </nuxt-link>
      </section>
      <section
        ...
        <lfx-icon name="book" :size="40" class="text-neutral-300" />
        <p class="pt-5 text-sm leading-5 text-neutral-500 text-center">
      ...
      <section v-if="!tab || tab === 'collections' && props.collections.length > 0" class="flex flex-col gap-1">
        <nuxt-link
          ...
          class="px-3 py-2 rounded-md transition-all
        hover:bg-neutral-50 flex items-center gap-2 cursor-pointer text-sm text-neutral-900">
          <lfx-icon name="rectangle-history" :size="16" class="text-neutral-400" />
          {{collection.name}}
        </nuxt-link>
      </section>
      <section
        ...
        class="px-3 py-12 flex flex-col items-center">
        <lfx-icon name="rectangle-history" :size="40" class="text-neutral-300" />
        <p class="pt-5 text-sm leading-5 text-neutral-500 text-center">

...
export function useRichSchema() {
  ...
  const getProjectSchema = (projectRef: Ref<Project | undefined>) => {
    return computed(() => {
      if (!projectRef.value) return { script: [] };
      ...
      const schemaGraph: Record<string, unknown>[] = [
        {
          ...
          url: projectUrl,
          description:
            `Project contributors, popularity, development, and security metrics ` +
  ...
  const getCollectionSchema = (collectionRef: Ref<Collection | undefined>) => {
    return computed(() => {

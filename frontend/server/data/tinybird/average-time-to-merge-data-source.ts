...
export async function fetchAverageTimeToMerge(
  filter: AverageTimeToMergeFilter,
): Promise<AverageTimeMerge> {
  ...
  const [currentSummary, previousSummary, averageTimeToMergeData] = await Promise.all([

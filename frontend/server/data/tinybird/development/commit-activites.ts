...
function getTinybirdQueries(filter: ActivityCountFilter) {
  ...
  return {
    currentSummaryQuery: {
      ...
      activity_types: filter.activity_types || (filter.activity_type ? [filter.activity_type] : undefined),
...
export async function fetchCommitActivities(
  filter: ActivityCountFilter,
): Promise<CommitActivities> {
  ...
  if (filter.countType === ActivityFilterCountType.CUMULATIVE) {
    data = currentData.data.map((item) => ({
      ...
      commits: item.cumulativeActivityCount || 0,
    }));
  } else {
    data = currentData.data.map((item) => ({
      startDate: item.startDate,
      endDate: item.endDate,
      commits: item.activityCount || 0,
    }));
  }

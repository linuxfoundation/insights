...
describe('Issues Resolution Data Source', () => {
  ...
  test('should fetch issues resolution data with correct parameters', async () => {
    ...
    const {fetchIssuesResolution} = await import("~~/server/data/tinybird/development/issues-resolution-data-source");
    const { mergeRanges } = await import('~~/server/data/tinybird/development/issues-resolution-data-source');

...
export async function fetchReviewTimeByPRSize(
  filter: ReviewTimeByPRSizeFilter,
): Promise<ReviewTimeByPrItem[]> {
  const path = '/v0/pipes/pull_requests_review_time_by_size.json';

  const waitReviewTimeByPRSizeData = await fetchFromTinybird<TinybirdReviewTimeByPRSizeData[]>(
    path,
    filter,
  );

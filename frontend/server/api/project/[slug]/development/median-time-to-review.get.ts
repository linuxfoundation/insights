...
import type { MedianTimeToReviewFilter } from '~~/server/data/types';
 ...
 *     medianTime: number; // median time to first review in seconds
 *   }[];
 * }
 */
export default defineEventHandler(async (event) => {
  ...
  const filter: MedianTimeToReviewFilter = {
    project,
  ...
  return await dataSource.fetchMedianTimeToReview(filter);
});

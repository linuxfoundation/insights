...
export default defineEventHandler(async (event) => {
  const { projectSlug, repositorySlug, repositoryGroupSlug } = getQuery(event) as Record<
    string,
    string
  >;
  ...
  try {
    ...
    if (repositoryGroupSlug) {
      const repositoryGroups = await fetchFromTinybird<ProjectRepositoryGroup[]>(
        '/v0/pipes/repository_groups_list.json',
        {
          project: projectSlug,
        },
      );
...
function svgTemplate(
  project: ProjectTinybird,
  repository: ProjectRepository | undefined,
  repositoryGroup?: ProjectRepositoryGroup,
) {
  ...
  if (repositoryGroup) {
    repositoryName =
      repositoryGroup.name.length > 40
        ? `${repositoryGroup.name.substring(0, 39)}...`

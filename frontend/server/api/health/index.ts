...
export default defineEventHandler(async (): Promise<HealthCheckResponse> => {
  try {
    const pingTinybirdResult = await fetchFromTinybird<PingResult[]>('/v0/pipes/ping.json', {});
    const isTinybirdReachable =
      pingTinybirdResult.data.length > 0 && pingTinybirdResult.data[0].result === 'pong';

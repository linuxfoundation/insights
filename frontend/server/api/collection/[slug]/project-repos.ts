 ...
 * Method: GET
 * Description: Fetches paginated project and repository insights for a specific collection.
 *
 * URL Parameters:
 ...
 * - type (string, optional): Filter by record type ("project" or "repo").
 ...
 * - data (Array): The list of project and repository insights.
 *
 * Errors:
...
export default defineEventHandler(async (event): Promise<Pagination<unknown> | Error> => {
  ...
  try {
    ...
    const { projectIds, repositoryUrls } = result;

    if (projectIds.length === 0 && repositoryUrls.length === 0) {
      return {
        page,
        pageSize,
        total: 0,
    ...
    if (repositoryUrls.length > 0) {
      params.repoUrls = repositoryUrls;
    }
    ...
    const response = await postToTinybird<ProjectInsightsTinybird[]>(
      '/v0/pipes/project_repo_insights.json',
      params,
    );
    ...
    console.error('Error fetching collection project-repos:', error);

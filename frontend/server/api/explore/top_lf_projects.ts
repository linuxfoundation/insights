...
/**
 * API Endpoint: /api/explore/top_lf_projects
 * Method: GET
 * Description: Fetches the list of top Linux Foundation projects, specifically for the "Top Linux Foundation projects"
 * widget. This is basically a copy of server/api/project/index.ts, but calling a different Tinybird endpoint. I'm
 * leaving the parameters unchanged in relation to the original endpoint handler because:
 *   1. I'm not entirely certain that we don't want to use this to get data for other parts of the app, and thus we will
 *      need to make this more generic.
 *   2. I'm also not sure that this is the best approach for what we want to do, so this is more like an experiment and
...
export default defineEventHandler(async (event): Promise<Pagination<ProjectList> | Error> => {
  ...
  try {
    const res = await fetchFromTinybird<ProjectTinybird[]>('/v0/pipes/projects_list_for_top_projects_widget.json', {
      pageSize,
    })

...
export async function persistSearchVolume(
  store: DbStore,
  records: SearchVolumeDBRecord[],
): Promise<void> {
  if (records.length === 0) {
  ...
  console.info(
    `Finished saving search volume data: ${successCount} successful, ${errorCount} failed out of ${records.length} total records.`,
  );
}

...
async function getSearchVolume(
  ...
  const { fetcher = ofetch, apiKey = getDefaultApiKey(), apiUrl = DEFAULT_API_URL } = options;
  ...
  try {
    ...
    console.log(
      `API call successful. Credits consumed: ${result.credits_consumed ?? 'N/A'}, Credits remaining: ${result.credits ?? 'N/A'}`,
    );
    return result;
  } catch (error: any) {

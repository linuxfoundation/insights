...
14. **Billing model:** bundled with existing LFX membership. No standalone billing infra in v1. Tier comes from the existing LFX tier on the user's org. Usage is metered only for rate-limit enforcement and Datadog dashboards — not for invoicing. Enforcement is split: Self-Serve validates membership at token-mint time (OpenFGA `v2_organization` entities — confirm at T-015) and bakes `tier` + `org` into the access token; the Insights API reads those claims on every request to enforce rate limits and future per-endpoint tier gating, but never re-queries any membership system. See ADR-0010.

...
The customer-facing credential is a Personal Access Token issued by the LFX Self-Serve App. A Cloudflare Worker in front of the Insights API exchanges it, per request, for a short-lived Auth0-signed JWT via Auth0 Custom Token Exchange, resolves the caller's org and membership tier from an LFX Tier endpoint, and forwards both to the Insights API as trusted headers. The Insights API verifies the JWT and never sees the PAT. No client-side token-swap call, no Insights-hosted token endpoint, and the same Worker handles plain OAuth2 access tokens — so the MCP and CLI phases reuse this path unchanged.

Delivery is owned by the Insights team (approved architecture; engineering owns delivery): we build the PAT exchange service and the LFX Tier endpoint, and steward coordination with DevOps, Platform/Self-Serve, SSO, and the LFX v2 platform team. Variant 4b (tier resolved by the Worker) is what we plan to build; 4a (tier enriched by the PAT service) stays viable. Full details, including the coordination table, in [docs/adr/0006](../adr/0006-pat-token-exchange-for-api-credentials.md).

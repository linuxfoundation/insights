...
- Heavy observability (OTel → Datadog) so we can offer SLAs and enforce tier-based rate limits confidently.
- Phased rollout: **Development → Contributors → Popularity → Security & Best Practices → Overviews → Collections → Leaderboard** (more later).
...
- **T-030** Embed Scalar on the reference page; wire it to ingest the generated per-version OpenAPI spec (the same artifact served at `/v1/openapi.json`, per T-037) on every release. Serve the static VitePress build at `api.insights.linuxfoundation.org/docs` via Fastify's static file serving under `/docs`.
...
11. **Data scope (v1):**
    - **Phases 1–5 (Development, Contributors, Popularity, Security & Best Practices, Overviews):** public OSS data, **no per-project permission check**. Tier check only.
    - **Phase 6 (Collections):** adds a tier check + permission check (private collections gated by the owner-only `collections.ssoUserId == sub` check per [ADR-0007](adr/0007-collections-only-permission-check.md); public collections open to all valid keys). Permission source: **Postgres lookup with Redis cache** (~60s TTL).
...
- Tinybird errors: serve the cached Redis response if one exists within its normal TTL; if the cache is empty or expired, 503 with `code: upstream_unavailable`. (see CONTEXT.md conventions)

...
2. Because the Cloudflare Worker performs the token exchange against Auth0 (per ADR-0006), the PAT service holds no Auth0 client credentials for the exchange itself. It needs durable storage, CRUD for token lifecycle, and a fast validation endpoint. Creating an Insights PAT is gated on Key Contact status ([ADR-0010](./0010-billing-bundled-with-lfx-membership.md)); that gate is enforced by lfx-one before the create request is proxied to the PAT service (per ADR-0006 step 1), so the PAT service holds no Auth0 M2M client credentials for membership lookup. The absence of Auth0 Management API needs removes the original argument for extending lfx-v2-auth-service, whose value is exactly that integration.
...
- **Cons**: The Worker, not the PAT service, performs the exchange, so auth-service's Auth0 Management API machinery stays unused; the PAT service holds no Auth0 credentials at all. PAT storage and lifecycle share no domain logic with identity management. It couples an Insights-delivered feature to another team's service, roadmap, and release cadence.
...
- Zero changes to lfx-v2-auth-service; auth0-terraform gains the CTE profile and the action that calls the validation endpoint. The PAT service requires no M2M application in Auth0.

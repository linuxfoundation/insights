...
const route = '/v1-alpha/projects/kubernetes/popularity/forks';
...
const bucket = (startDate: string, endDate: string, forks: number) => ({
...
describe('response (AC1, AC5)', () => {
  it('returns the signed period summary and new forks per bucket by default', async () => {
...
describe('Tinybird calls (AC2)', () => {
  it('counts forks on every call with the Nuxt flags and no platform filter', async () => {
...
describe('countType=cumulative (AC3)', () => {
  it('returns the running total per bucket and keeps the summary on forks added in the period', async () => {
...
describe('fixed activity type (AC4)', () => {
  it('ignores an activityType query parameter', async () => {
    const res = await get(`${route}?${baseQuery}&activityType=star`);
    expect(res.statusCode).toBe(200);
    for (const url of pipeCalls()) {
      expect(url.searchParams.get('activity_type')).toBe('fork');
    }
  });
});
...
describe('OpenAPI (AC6)', () => {
  async function getOperation(): Promise<OpenApiOperation> {
    ...
    const operation = res.json<OpenApiDoc>().paths[projectPath('popularity/forks')]?.get;
    expect(operation, 'forks is missing from the spec').toBeDefined();
    return operation as OpenApiOperation;
  }

  it('documents countType as an optional new or cumulative switch and forks as an integer', async () => {
    ...
    expect(schema?.properties?.data?.items?.properties?.forks?.type).toBe('integer');
  });
});

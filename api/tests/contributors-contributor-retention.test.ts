...
const url = (params: Record<string, string | string[] | undefined> = {}, slug = 'kubernetes') =>
  `/v1-alpha/projects/${slug}/contributors/contributor-retention?${queryString({
    startDate,
    endDate,
    granularity: 'monthly',
    ...params,
...
describe('GET /v1-alpha/projects/{slug}/contributors/contributor-retention (CR1)', () => {
...
describe('Tinybird calls (CR3)', () => {
  it('makes a single contributor_retention call with the slug as project, onlyContributions=false and both flags at their defaults', async () => {
    mockFetch.mockImplementation(routeTinybird());
    await get(url());
    ...
    expect(call.pathname).toBe('/v0/pipes/contributor_retention.json');

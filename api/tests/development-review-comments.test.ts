...
const specPath = '/v1-alpha/projects/{slug}/development/review-comments';
...
function url(params: Record<string, string | string[] | undefined> = {}, slug = 'kubernetes') {
  ...
  return `/v1-alpha/projects/${slug}/development/review-comments?${search}`;
}
...
describe('Tinybird calls (AC2)', () => {
  it('makes three activities_count calls with the slug, the repos and the review comment activity types', async () => {
...
describe('request validation (AC6)', () => {
  ...
  // The Nuxt handler fell back to quarterly; the API has no default.
  it('rejects a missing granularity instead of defaulting it', async () => {
...
describe('OpenAPI (AC9)', () => {
  ...
  it('documents every field of the 200 response', async () => {
    ...
    const bucketFields = ['startDate', 'endDate', 'reviewComments'];
    ...
    expect(bucket?.properties?.reviewComments?.type).toBe('integer');
  ...
  it('keeps the route out of /v1', async () => {
    ...
    expect(Object.keys(spec.json<OpenApiDoc>().paths)).not.toContain(
      '/v1/projects/{slug}/development/review-comments',
    );
    const res = await get(
      '/v1/projects/kubernetes/development/review-comments?granularity=monthly',
    );

...
3. Run `git branch --show-current` to get the branch name. If it contains an `IN-XXX` ticket number, include it inline at the end of the subject (e.g. `feat: add token refresh IN-123`). If no ticket is found in the branch name, use `AskUserQuestion` to ask for the JIRA ticket number before committing — per `commit-workflow.md`, commits should include one.

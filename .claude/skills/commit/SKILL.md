...
4. Generate a single commit message following Conventional Commits format: `type: description`. Subject max 72 characters. No scope required. No body, no extra trailers, no Co-Authored-By. (The `Signed-off-by` trailer is added automatically by `-s`.)
